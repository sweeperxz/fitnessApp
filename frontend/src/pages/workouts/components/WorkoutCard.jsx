    ...
    <div className="workout-card">
      <div className="workout-card-header">
        <div>
          ...
          {workout.notes && <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 3 }}>{workout.notes}</div>}
        </div>
        <button
      ...
      {workout.exercises.map(exercise => (
        <div key={exercise.id} className="exercise-row">
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 2 }}>{exercise.sets} × {exercise.reps} {repsLabel}</div>
          </div>
